...
DEBUG_PARAMS="--max_eval_samples 25 --num_inference_diffusion_steps 10 --gradient_accumulation_steps 1 "
...
# MODEL_NAME="ul2"
# truncation_length=206
# python -m torch.distributed.launch --nproc_per_node 2 run_mlm.py --truncation_length ${truncation_length} --model_name_or_path ${MODEL_PATH} --do_train --output_dir ${output_dir}"/test1" --num_inference_diffusion_steps ${num_inference_diffusion_steps}  ${shared_params} --tokenized_data_path ${tokenized_data_path} --top_p ${TOP_P} ${DEBUG_PARAMS} --save_steps 20 --eval_steps 100 # --resume_from_checkpoint "/net/nfs.cirrascale/s2-research/rabeehk/outputs/paper_experiments/ours_eval//test/checkpoint-80/"
