...
def predict_conditional_generated(
    ...
    pred_masked_texts = [
        tokenizer.batch_decode(x[:x.index(tokenizer.eos_token_id)], skip_special_tokens=True) for x in masked
    ]
    pred_unmasked_texts = [
        tokenizer.batch_decode(x[:x.index(tokenizer.eos_token_id)], skip_special_tokens=True) for x in unmasked
    ]
