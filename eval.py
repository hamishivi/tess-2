...
def predict_conditional_generated(batch, tokenizer, predicted_token_ids, prefix_name):
    masked = list(map(lambda x, y: split_into_masked_and_unmasked(x, y, return_masked=True), predicted_token_ids, batch["span_mask"]))
    unmasked = list(map(lambda x, y: split_into_masked_and_unmasked(x, y, return_masked=False), batch["input_ids"], batch["span_mask"]))
    ...
    pred_texts_marked = list(map(lambda x, y: concatenate_alternatively(x, y, mark="***"), pred_unmasked_texts, pred_masked_texts)) 
...
def generate_text(pipeline, tokenizer, diffusion_args, training_args, data_args, accelerator, batch=None):
    ...
    if data_args.span_infilling:
        ...
        results.update({"pred_texts_from_simplex": tokenizer.batch_decode(token_ids_from_simplex, skip_special_tokens=False)})
