...
def main():
    parser = HfArgumentParser(
        (DataTrainingArguments, TrainingArguments, ModelArguments, DiffusionArguments)
    )
    if len(sys.argv) == 2 and sys.argv[1].endswith(".json"):
        data_args, training_args, model_args, diffusion_args = parser.parse_json_file(
    ...
    roberta_tokenizer = AutoTokenizer.from_pretrained(
        "roberta-large", **tokenizer_kwargs
    )
    ...
    if data_args.tokenized_data_path:
        ...
        tokenized_datasets = tokenize_data_new(
            data_args, roberta_tokenizer, raw_datasets, training_args
        )
    ...
    with torch.no_grad():
        for i, batch in enumerate(eval_dataloader):
            ...
            prefixes = roberta_tokenizer.batch_decode(
                prefixes, skip_special_tokens=True
            )
            ...
            generated_outputs = [
                output[len(prefix) :]
                for output, prefix in zip(outputs, prefixes_inputs["input_ids"])
            ]
            generated_outputs = [
                tokens.cpu().numpy().tolist() for tokens in generated_outputs
            ]
            all_outputs.extend(generated_outputs)
    results = {}
    generated_texts = [
        tokenizer.decode(output, skip_special_tokens=True) for output in all_outputs
    ]
    gold_texts = [
        roberta_tokenizer.decode(input[mask], skip_special_tokens=True)
        for input, mask in zip(all_inputs, all_masks)
    ]
    total_texts_marked = [
        prefix + " ***" + generated_text + "***"
        for prefix, generated_text in zip(all_prefixes, generated_texts)
    ]
    results = {
