...
def main():
    ...
    if len(sys.argv) == 2 and sys.argv[1].endswith(".json"):
        data_args, training_args, model_args = parser.parse_json_file(
    ...
    with torch.no_grad():
        for i, batch in enumerate(eval_dataloader):
            ...
            prefixes = roberta_tokenizer.batch_decode(
                prefixes_tokens, skip_special_tokens=True
            )
            all_prefixes.extend(prefixes)
            # Note that output also include the prefix and we need to remove it here.
            gold_texts = [
                input[len(prefix) :] for input, prefix in zip(inputs, prefixes_tokens)
            ]
