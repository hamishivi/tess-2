...
def main():
    parser = HfArgumentParser(
        (ModelArguments, DataTrainingArguments, BaselineSeq2SeqTrainingArguments)
    )
    ...
    def compute_metrics(eval_preds, split="eval"):
        ...
        decoded_labels_original = tokenizer.batch_decode(
        ...
        if split == "eval":
            ...
            sources = tokenizer.batch_decode(
                predict_input_ids, skip_special_tokens=True
            )
        metrics = {}
        for metric_name, metric in eval_metrics.items():
            scale = 100 if metric_name != "sari" else 1
            if metric_name == "sari":
                decoded_preds, decoded_labels, sources = postprocess_text_for_metric(
                ...
                decoded_preds, decoded_labels = postprocess_text_for_metric(
                    "bertscore_them", decoded_preds_original, decoded_labels_original
                )
                key_metrics = {
