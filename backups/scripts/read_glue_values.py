...
def read_values(paths, is_baseline=False, tasks=None):
    ...
    for task in ALL_TASKS:
        ...
        for metric in task_to_metric[task]:
            if is_baseline:
                ...
                scale = 1
                results[task][metric] = np.round(
                    data["eval_pred_texts_from_logits_masked_" + metric] * scale, 2
                )
    print(results)
    ...
    for task in results:
        metric = (
            task_to_metric[task][0]
            if len(task_to_metric[task]) == 1
            else "combined_score"
        )
...
for task in task_to_metric.keys():
    ...
    paths[task] = os.path.join(
        path_task,
        "eval_top_p_None_temperature_1.0_seed_42_guidance_scale_1.0_results.json",
