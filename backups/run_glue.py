...
def main():
    ...
    def preprocess_function(examples):
        ...
        # Tokenize the labels.
        targets = [
            str(round_stsb_target(label)) if is_regression else str(label)
        ...
        if data_args.add_t5_tags:
            sentence1_with_tag = [
                sentence1_key + ": " + sentence_1
                for sentence_1 in examples[sentence1_key]
            ]
            if sentence2_key is not None:
                sentence2_with_tag = [
                    sentence2_key + ": " + sentence_2
                    for sentence_2 in examples[sentence2_key]
                ]
            args = (
                (sentence1_with_tag,)
                if sentence2_key is None
                else (sentence1_with_tag, sentence2_with_tag)
            )
        else:
            args = (
